...
@section('breadcrumb')
    ...
    <li class="breadcrumb-item">{{ __('Edit Plot') }}</li>
@endsection

@section('content')
    <form action="{{ route('construction-project.plot.update', $plot->id) }}" method="POST" enctype="multipart/form-data">
        @csrf @method('PUT')
        <div class="row">
            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header bg-primary text-white">
                        <h5 class="mb-0"><i class="ti ti-layout-grid"></i> {{ __('Plot Information') }}</h5>
                    </div>
                    <div class="card-body">
                        @if ($errors->any())
                        ...
                        <div class="row">
                            <div class="col-md-3">
                                <div class="form-group mb-3">
                                    <label class="form-label">{{ __('Plot Number') }} <span
                            ...
                            <div class="col-md-2">
                                <div class="form-group mb-3">
                                    <label class="form-label">{{ __('Area') }} <span
                                            class="text-danger">*</span></label>
                                ...
                                <div class="form-group mb-3">
                                    ...
                                    <select name="area_unit" class="form-control">
                                        @foreach (['Marla', 'Kanal', 'Acre', 'Sq Ft'] as $unit)
                                            <option value="{{ $unit }}"
                                                {{ old('area_unit', $plot->area_unit) == $unit ? 'selected' : '' }}>
                                                {{ $unit }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                ...
                                <div class="form-group mb-3">
                                    ...
                                    <select name="status" class="form-control">
                                        <option value="available"
                                            {{ old('status', $plot->status) == 'available' ? 'selected' : '' }}>🔴 Available
                                        </option>
                                        <option value="sold"
                                            {{ old('status', $plot->status) == 'sold' ? 'selected' : '' }}>🟢 Sold
                                        </option>
                                    </select>
                                </div>
                            </div>
                            <div class="col-md-4">
                                <div class="form-group mb-3">
                                    ...
                                    <input type="number" name="amount" class="form-control" required
                                        value="{{ old('amount', $plot->amount) }}" step="0.01">
                                </div>
                            </div>
                            <div class="col-md-4">
                                <div class="form-group mb-3">
                                    ...
                                    <input type="text" name="latitude" id="field_lat" class="form-control"
                                        value="{{ old('latitude', $plot->latitude) }}" readonly>
                                </div>
                            </div>
                            <div class="col-md-4">
                                <div class="form-group mb-3">
                                    ...
                                    <input type="text" name="longitude" id="field_lng" class="form-control"
                                        value="{{ old('longitude', $plot->longitude) }}" readonly>
                                </div>
                            </div>
                        </div>
                        {{-- Leaflet Map Picker --}}
                        <div class="mb-2">
                            <div class="d-flex justify-content-between align-items-center mb-2">
                                <label class="form-label mb-0">{{ __('Pick Plot Location on Map') }}</label>
                            ...
                            <small id="map-hint"
                                class="text-muted">{{ __('Click on map to update plot location') }}</small>
                        </div>
                    </div>
                </div>
            </div>
                    ...
                    <div class="card-body">
                        ...
                        <div class="form-group mb-3">
                            ...
                            <input type="text" name="purchaser_cnic" class="form-control"
                                placeholder="xxxxx-xxxxxxx-x" value="{{ old('purchaser_cnic', $plot->purchaser_cnic) }}">
                        </div>
                        <div class="form-group mb-3">
                        ...
                        <div class="form-group mb-3">
                            ...
                            <textarea name="purchaser_address" class="form-control" rows="2">{{ old('purchaser_address', $plot->purchaser_address) }}</textarea>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <div class="form-group mb-3">
                            ...
                            <textarea name="agent_address" class="form-control" rows="2">{{ old('agent_address', $plot->agent_address) }}</textarea>
                        </div>
                        <div class="form-group mb-3">
                    ...
                    <div class="card-body">
                        <div class="form-group mb-3">
                            ...
                            <input type="number" name="patwari_total" class="form-control" step="0.01"
                                value="{{ old('patwari_total', $plot->patwari_total) }}">
                        </div>
            ...
            <div class="col-md-6">
                <div class="card">
                    ...
                    <div class="card-body">
                        @php
                            $existingDocs = \App\Models\RealEstateDocument::where('model_type', 'construction_plot')
                                ->where('model_id', $plot->id)
                                ->get();
                        @endphp
