                            ...
                            <tbody>
                                @forelse($mouzas as $mouza)
                                    <tr>
                                        ...
                                        <td>{{ $mouza->tehsil ?? '-' }}</td>
                                        <td>{{ $mouza->area_display }}</td>
                                        <td><span class="badge bg-info">{{ $mouza->fields_count }}</span></td>
                                        <td><span class="badge bg-danger">🔴 {{ $mouza->available_fields_count }}</span>
                                        </td>
                                        <td><span class="badge bg-success">🟢 {{ $mouza->sold_fields_count }}</span></td>
                                        <td>
                                            <a href="{{ route('mouza.show', $mouza->id) }}" class="btn btn-sm btn-info"
                                                title="View & Map">
                                            ...
                                            <form action="{{ route('mouza.destroy', $mouza->id) }}" method="POST"
                                ...
                                @empty
                                    <tr>
                                        <td colspan="8" class="text-center">{{ __('No Mouza found.') }}</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
