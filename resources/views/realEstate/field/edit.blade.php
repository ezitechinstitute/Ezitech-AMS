...
@section('page-title')
    {{ __('Edit Khait - {{ $field->mouza->name }}
@endsection
@section('breadcrumb')
    ...
    <li class="breadcrumb-item">{{ __('Edit Khait - {{ $field->mouza->name }}
@endsection

@section('content')
<form action="{{ route('mouza.field.update', $field->id) }}"  method="POST" enctype="multipart/form-data">
@csrf
@method('PUT')
<div class="row">
        ...
        <div class="card">
            ...
            <div class="card-body">
                ...
                <div class="row">
                    <div class="col-md-3">
                        <div class="form-group mb-3">
                            ...
                            <input type="text" name="field_number" class="form-control" required value="{{ old('field_number') }}">
                        </div>
                    </div>
                    <div class="col-md-3">
                        <div class="form-group mb-3">
                            <label class="form-label">{{ __('Intiqal No.') }}</label>
                            <input type="text" name="intiqal_no" class="form-control" value="{{ old('intiqal_no') }}">
                        </div>
                    </div>
                    <div class="col-md-2">
                        <div class="form-group mb-3">
                            <label class="form-label">{{ __('Land Area') }} <span class="text-danger">*</span></label>
                            <input type="text" name="area_quantity" class="form-control" required value="{{ old('area_quantity') }}" placeholder="e.g. 5">
                        </div>
                    </div>
                    <div class="col-md-2">
                        <div class="form-group mb-3">
                            ...
                            <select name="area_unit" class="form-control">
                                ...
                                <option value="Sq Ft" {{ old('area_unit') == 'Sq Ft' ? 'selected':'' }}>Sq Ft</option>
                            </select>
                        </div>
                    </div>
                    <div class="col-md-2">
                    ...
                    <div class="col-md-4">
                        <div class="form-group mb-3">
                            <label class="form-label">{{ __('Deal Amount (PKR)') }} <span class="text-danger">*</span></label>
                            <input type="number" name="amount" class="form-control" required value="{{ old('amount') }}" step="0.01">
                        </div>
                    </div>
                    <div class="col-md-4">
                        <div class="form-group mb-3">
                            <label class="form-label">{{ __('Latitude') }}</label>
                            <input type="text" name="latitude" id="field_lat" class="form-control" value="{{ old('latitude') }}" placeholder="For map pin">
                        </div>
                    </div>
                    <div class="col-md-4">
                        <div class="form-group mb-3">
                            <label class="form-label">{{ __('Longitude') }}</label>
                            <input type="text" name="longitude" id="field_lng" class="form-control" value="{{ old('longitude') }}" placeholder="For map pin">
                        </div>
                    </div>
                </div>
                {{-- Map Picker --}}
                <div class="mb-2">
                    ...
                    <div id="field-map-picker" style="height:280px; border-radius:8px; border:1px solid #ddd;"></div>
                    <small class="text-muted">{{ __('Click on map to pin this field location') }}</small>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="col-md-6">
        <div class="card">
            ...
            <div class="card-body">
                <div class="form-group mb-3">
                    <label class="form-label">{{ __('Seller Full Name') }} <span class="text-danger">*</span></label>
                    <input type="text" name="seller_name" class="form-control" required value="{{ old('seller_name') }}">
                </div>
                <div class="form-group mb-3">
                    <label class="form-label">{{ __('Father Name') }}</label>
                    <input type="text" name="seller_father_name" class="form-control" value="{{ old('seller_father_name') }}">
                </div>
                <div class="form-group mb-3">
                    <label class="form-label">{{ __('CNIC No.') }}</label>
                    <input type="text" name="seller_cnic" class="form-control" placeholder="xxxxx-xxxxxxx-x" value="{{ old('seller_cnic') }}">
                </div>
                <div class="form-group mb-3">
                    <label class="form-label">{{ __('Phone') }}</label>
                    <input type="text" name="seller_phone" class="form-control" value="{{ old('seller_phone') }}">
                </div>
                <div class="form-group mb-3">
        ...
        <div class="card">
            ...
            <div class="card-body">
                ...
                <div class="form-group mb-3">
                    ...
                    <input type="text" name="agent_cnic" class="form-control" placeholder="xxxxx-xxxxxxx-x" value="{{ old('agent_cnic') }}">
                </div>
                <div class="form-group mb-3">
                ...
                <div class="form-group mb-3">
                    ...
                    <input type="number" name="agent_commission" class="form-control" step="0.01" value="{{ old('agent_commission', 0) }}">
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="card">
            ...
            <div class="card-body">
                <div class="form-group mb-3">
                    ...
                    <input type="number" name="patwari_total" id="patwari_total" class="form-control" step="0.01" value="{{ old('patwari_total', 0) }}">
                </div>
                ...
                <div class="table-responsive">
                    <table class="table table-bordered" id="patwari-table">
                        ...
                        <tr>
                            ...
                            <th>
                                <button type="button" class="btn btn-sm btn-success" onclick="addPatwariRow()">
        ...
        <div class="card">
            ...
            <div class="card-body">
                ...
                <small class="text-muted">
                    ...
                    <a href="{{ route('bank-account.create') }}" target="_blank">{{ __('Add New Bank Account') }}</a>
            ...
            <div class="card-body">
                <div id="documents-wrapper">
                    <div class="doc-row row mb-2 align-items-center">
                        ...
                        <div class="col-4">
                            <input type="text" name="document_names[]" class="form-control form-control-sm" placeholder="Document Name">
                        </div>
                        <div class="col-3">
                            <input type="file" name="documents[]" class="form-control form-control-sm">
                        </div>
                        <div class="col-1">
                            <button type="button" class="btn btn-sm btn-success" onclick="addDocRow()"><i class="ti ti-plus"></i></button>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <div class="col-12">
        <div class="card">
            <div class="card-body">
                ...
                <div class="text-end">
                    <a href="{{ route('mouza.show', $mouza->id) }}" class="btn btn-secondary">{{ __('Cancel') }}</a>
                    <button type="submit" class="btn btn-primary"><i class="ti ti-check"></i> {{ __('Save Khait') }}</button>
                </div>
            </div>
        </div>
    </div>
...
function initFieldMap() {
    var lat = parseFloat(document.getElementById('field_lat').value) || {{ $mouza->latitude ?? 31.5204 }};
    var lng = parseFloat(document.getElementById('field_lng').value) || {{ $mouza->longitude ?? 74.3587 }};
...
<script src="https://maps.googleapis.com/maps/api/js?key={{ env('GOOGLE_MAPS_API_KEY', '') }}&callback=initFieldMap" async defer></script>
@endpush
